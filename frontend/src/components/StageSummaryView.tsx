...
const TYPE_BADGE: Record<string, { label: string; color: string }> = {
  mountain: { label: 'MTN', color: 'text-blue-400 bg-blue-950/60' },
  flat:     { label: 'FLT', color: 'text-emerald-400 bg-emerald-950/60' },
  hilly:    { label: 'HLY', color: 'text-amber-400 bg-amber-950/60' },
  tt:       { label: 'TT',  color: 'text-purple-400 bg-purple-950/60' },
};
      ...
      <table className="w-full border-collapse text-xs">
        <thead>
          <tr className="border-b border-border">
            ...
            <th className="text-left py-2 px-3 text-[10px] font-semibold text-amber-500/70 uppercase tracking-wider">Favourite</th>
        ...
        <tbody>
          {stages.map((s) => {
            const badge = TYPE_BADGE[s.type] ?? { label: s.type.toUpperCase(), color: 'text-gray-400 bg-gray-800' };
            return (
              <tr
                key={s.stage}
                className="border-b border-border/40 hover:bg-card/50 transition-colors"
              >
                <td className="py-1.5 px-3 font-mono text-gray-400 font-semibold">{s.stage}</td>
                ...
                <td className="py-1.5 px-2">
                  <div className="text-gray-200 font-medium">{s.finish}</div>
                  {s.key_climbs.length > 0 && (
                    <div className="text-[9px] text-gray-600 truncate max-w-[160px]" title={s.key_climbs.join(', ')}>
                      {s.key_climbs.join(' · ')}
                    </div>
                  )}
                </td>
                ...
                {[0, 1, 2, 3, 4].map((rank) => {
                  ...
                  if (!rider) return <td key={rank} className="py-1.5 px-3 text-gray-700">—</td>;
                  return (
                    <td key={rank} className="py-1.5 px-3">
                      <div className={rank === 0 ? 'text-white font-semibold' : 'text-gray-400'}>
                        {rider.name.split(' ').pop()}
                      </div>
                      <div className={`font-mono text-[9px] ${rank === 0 ? 'text-amber-400' : 'text-gray-600'}`}>
