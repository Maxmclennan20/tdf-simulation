  ...
  return (
    <tr className={`border-b border-border/40 hover:bg-card/50 transition-colors ${inactive ? 'opacity-40' : ''}`}>
      <td className="py-1 pl-3 pr-1">
        <span className="block truncate text-gray-900 font-medium text-xs" title={rider.name}>{rider.name}</span>
      </td>
      <td className="py-1 px-1">
        <span className="block truncate text-gray-600 text-xs" title={rider.team}>{rider.team}</span>
      </td>
      {RATINGS.map(attr => (
      ...
      <td className="py-1 px-1 text-center">
        <input
          ...
          style={{ color: rider.form > 1.1 ? '#059669' : rider.form < 0.9 ? '#dc2626' : '#6b7280' }}
